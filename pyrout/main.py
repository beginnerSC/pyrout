...
def rsdr(residuals: List[float], n_params_fit: int = 0) -> float:
    """
    ...
    residuals : array-like
        The residuals of the data values after fitting the model.
...
def lorentzian_merit(params: List[float], model_func, data_points: List[Tuple[float, float]]) -> float:
    """
    Computes the Lorentzian merit given data points and the model function to fit.
    ...
    data_points : array-like
        List of data points :math:`(x_i, y_i)` to be used for fitting the model function.
    ...
    """
    residuals = np.abs([(y - model_func(x, params)) for x, y in data_points])
    rsdr_val = rsdr(residuals, len(params))
...
def svi_black_scholes_call(K: float, r: float, a: float, b: float, rho: float, m: float, sigma: float) -> float:
    """
    ...
    -------
    float or array-like
        The computed call option prices corresponding to the given strike prices.
...
if __name__ == '__main__':
    ...
    np.random.seed(42)
    params_true = [1.0, -5.0, 10.0]  # True parameters for the quadratic model function
    data_points = []
    x_values = np.linspace(-10, 10, 100)
    for x in x_values:
        # Generate y values with some noise (simulating real data)
        y_true = quadratic_model(x, params_true)
        noise = np.random.normal(loc=0, scale=10)  # Simulate noise in the y value
        y_noisy = y_true + noise
        
        data_points.append((x, y_noisy))

    # Add some outliers to the data points
    outliers_indices = [20, 50, 80]  # Indices where outliers will be inserted
    outliers = [y_noisy + np.random.normal(loc=0, scale=50) for _ in outliers_indices]
    for i, index in enumerate(outliers_indices):
    ...
    y_values = np.array([y[1] for y in data_points])

    plt.scatter(x_values, y_values, label='Data points with outliers')
    plt.plot(x_values, quadratic_model(x_values, optimized_params), color='red', label='Fitted curve')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
    plt.grid(True)
    ...
    # bounds = [  (0, max_interest_rate),              # r
    #             (0, max_a),                          # a
    #             (-max_b, max_b),                     # b (allowing both positive and negative values)
    #             (-1, 1),                             # rho
    #             (min_m, max_m),                      # m
    #             (0, max_sigma)]                      # sigma

    # res = minimize(lorentzian_merit, np.zeros(n_params_fit), args=(model_func, data_points), bounds=bounds)
